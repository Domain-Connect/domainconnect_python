...
class TestDomainConnect(TestCase):
    def test_get_domain_connect_template_sync_url(self):
        for i in configs:
            with self.subTest(i = i):
                self._test_get_domain_connect_template_sync_url(i)

    def _test_get_domain_connect_template_sync_url(self, config: dict):
        ...
        assert (res == config['SYNC_URL'] + '/v2/domainTemplates/providers/exampleservice.domainconnect.org/services/template1/apply?domain=' + config['TEST_DOMAIN'] + '&host=&IP=132.148.25.185&RANDOMTEXT=shm%3A1531371203%3AHejo'), "URL is different than expected"

        #simple test sync with host
        res, error = dc.get_domain_connect_template_sync_url("justatest." + config['TEST_DOMAIN'], "exampleservice.domainconnect.org", "template1", params={"IP": "132.148.25.185", "RANDOMTEXT": "shm:1531371203:Hejo"})
        print(res)
        assert(error is None), "There is an error returned: {}".format(error)
        assert(res == config['SYNC_URL'] + '/v2/domainTemplates/providers/exampleservice.domainconnect.org/services/template1/apply?domain=' + config['TEST_DOMAIN'] + '&host=justatest&IP=132.148.25.185&RANDOMTEXT=shm%3A1531371203%3AHejo'), "URL is different than expected"

        #simple test sync with host and redirect uri and scope
        res, error = dc.get_domain_connect_template_sync_url("justatest." + config['TEST_DOMAIN'], "exampleservice.domainconnect.org", "template1", params={"IP": "132.148.25.185", "RANDOMTEXT": "shm:1531371203:Hejo"}, redirect_uri="http://google.com", state="{name=value}")
        print(res)
        assert(error is None), "There is an error returned: {}".format(error)
        assert(res == config['SYNC_URL'] + '/v2/domainTemplates/providers/exampleservice.domainconnect.org/services/template1/apply?domain=' + config['TEST_DOMAIN'] + '&host=justatest&IP=132.148.25.185&RANDOMTEXT=shm%3A1531371203%3AHejo&redirect_uri=http%3A%2F%2Fgoogle.com&state=%7Bname%3Dvalue%7D'), "URL is different than expected"
    ...
    def _test_get_domain_config(self, config: dict):
        dc = DomainConnect()
    ...
    def _test_get_domain_connect_template_async_url(self, config: dict):
        ...
        res, error = dc.get_domain_connect_template_async_url(config['TEST_DOMAIN'], "exampleservice.domainconnect.org", "template2",
        ...
        assert(res == config['ASYNC_URL'] + '/v2/domainTemplates/providers/exampleservice.domainconnect.org'
                         '?client_id=exampleservice.domainconnect.org&scope=template2&domain=' + config['TEST_DOMAIN'] +
        ...
        #simple test sync with host
        res, error = dc.get_domain_connect_template_async_url("justatest." + config['TEST_DOMAIN'], "exampleservice.domainconnect.org", "template2",
                                                       params={"IP": "132.148.25.185", "RANDOMTEXT": "shm:1531371203:Hejo"},
                                                       redirect_uri="https://exampleservice.domainconnect.org/async_oauth_response",
                                                       state="{name=value}")
        print(res)
